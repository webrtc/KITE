...
public class Remote extends KiteConfigObject {
    ...
    private final static String PATH_TO_DB = "KITE.db";
     ...
     * @throws KiteUnsupportedRemoteException if the 'name' is other than what is specified in SupportedRemote.
     */
    public Remote(JsonObject jsonObject) throws KiteUnsupportedRemoteException {
        ...
        if (this.isLocal()) {
            ...
            this.remoteAddress = SupportedRemote.valueOf(this.name).remoteAddress(this.username, this.accesskey);
        }
    }
     ...
     *
     * @return RemoteGridFetcher
     */
    public RemoteGridFetcher getGridFetcher() {
        switch (this.name) {
            case "saucelabs":
                return new SauceLabsGridFetcher(PATH_TO_DB, this.remoteAddress, SupportedRemote.saucelabs.restApiUrl());
            case "browserstack":
                return new BrowserStackGridFetcher(PATH_TO_DB, this.remoteAddress, SupportedRemote.browserstack.restApiUrl(), getUsername(), getAccesskey());
            case "testingbot":
                return new TestingBotGridFetcher(PATH_TO_DB, this.remoteAddress, SupportedRemote.testingbot.restApiUrl());
            default:
                return null;
        }
    }
