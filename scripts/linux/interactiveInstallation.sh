...
echo -e '\n'whether to use a desktop environment, or to run without any display
...
function configSelenium() {
	...
	case $ynq in
		[Nn]* )
			   ...
			   sed -i s/SELENIUM_VERSION_SHORT=$SELENIUM_VERSION_SHORT/SELENIUM_VERSION_SHORT=$InputSeleniumshort/ ./gridConfig.sh			   
			   echo Please enter the current version of Selenium \(complete version\)
			   read InputSelenium
			   sed -i s/SELENIUM_VERSION=$SELENIUM_VERSION/SELENIUM_VERSION=$InputSelenium/ ./gridConfig.sh
			   remind
			   ;;
		[Yy]* )
			   remind
...
function configFirefoxDriver() {
	...
	case $ynq in
		[Nn]* )
			   ...
			   sed -i s/GECKO_VERSION=$GECKO_VERSION/GECKO_VERSION=$InputGeckodriver/ ./gridConfig.sh
			   configSelenium
			   ;;
		[Yy]* )
			   configSelenium
...
function configChromeDriver() {
	...
	case $ynq in
		[Nn]* )
			   ...
			   sed -i s/CHROMEDRIVER_VERSION=$CHROMEDRIVER_VERSION/CHROMEDRIVER_VERSION=$InputChromeDriver/ ./gridConfig.sh
			   configFirefoxDriver
			   ;;
		[Yy]* )
			   configFirefoxDriver
...
function configFirefox() {
	...
	case $ynq in
		[Nn]* )
			   ...
			   sed -i s/FIREFOX_VERSION=$FIREFOX_VERSION/FIREFOX_VERSION=$InputFirefoxVersion/ ./gridConfig.sh
			   configChromeDriver
			   ;;
		[Yy]* )
			   configChromeDriver
...
function configChrome() {
	...
	case $ynq in
		[Nn]* )
			   ...
			   sed -i s/CHROME_VERSION=$CHROME_VERSION/CHROME_VERSION=$InputChromeVersion/ ./gridConfig.sh
			   configFirefox
			   ;;
		[Yy]* )
			   configFirefox
...
function localhost() {
	...
	case $ynq in
		[Yy]* )
			   ...
			   sed -i s/LOCALHOST=$LOCALHOST/LOCALHOST=TRUE/ ./gridConfig.sh
			   configChrome
			   ;;
		[Nn]* )
				echo You chose to use the IP as the hub address instead of 'localhost' 

			   sed -i s/LOCALHOST=$LOCALHOST/LOCALHOST=FALSE/ ./gridConfig.sh
			   configChrome
			  ;;
    [Qq]* )
           quit
...
function desktopEnvironment() {
	echo -e 
	read -p "Would you like to use a desktop Environment  or to run without any display? (y/n/q)" ynq
	case $ynq in
		[Yy]* )
			   sed -i s/DESKTOP_ENVIRONMENT=$DESKTOP_ENVIRONMENT/DESKTOP_ENVIRONMENT=TRUE/ ./gridConfig.sh
			   localhost
			   ;;
		[Nn]* )
			   sed -i s/DESKTOP_ENVIRONMENT=$DESKTOP_ENVIRONMENT/DESKTOP_ENVIRONMENT=FALSE/ ./gridConfig.sh
			   localhost
			  ;;
    [Qq]* )
           quit
...
function configGrid() {
	...
	case $ynq in
		[Nn]* )
			   ...
			   sed -i s/GRID_UTILS_VERSION=$GRID_UTILS_VERSION/GRID_UTILS_VERSION=$inputGridVersion/ ./gridConfig.sh
			   desktopEnvironment
			   ;;
		[Yy]* )
			   desktopEnvironment
