...
public class GetConfigProgressServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (strTestID == null) throw new KiteNoKeyException("id");
    if (log.isDebugEnabled()) log.debug("in->id: " + strTestID);
    int testID = Integer.parseInt(strTestID);
    String result = request.getParameter("result");
    if (result == null) throw new KiteNoKeyException("result");
    boolean resultBool = Boolean.parseBoolean(result);
    Test test;
    try {
      test = new TestDao(Utility.getDBConnection(this.getServletContext())).getTestById(testID);
/*      if (log.isDebugEnabled()) log.debug("testJson : " + test.getJsonData());
      else {
        if (resultBool) {
          listOfResult =
              new OverviewResult(
                  new ResultDao(Utility.getDBConnection(this.getServletContext()))
                      .getResultList(test.getResultTable(), test.getTupleSize()));
          if (log.isDebugEnabled())
            log.debug("listOfResultJson : " + listOfResult.getSunburstJsonData());
          if (test.getStatus()) response.getWriter().print("done");
          else response.getWriter().print(listOfResult.getSunburstJsonData());
        } else {
