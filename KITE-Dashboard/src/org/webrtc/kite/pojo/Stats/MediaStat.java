...
public class MediaStat {
            ...
            case "outbound-rtp":
              {
                ...
                for (String outbound_rtp_stream : outbound_rtp_stream_list) {
                  if (outbound_rtp_stream.toLowerCase().contains("outbound")
                      && outbound_rtp_stream.toLowerCase().contains(this.mediaType))
                    this.outbound_obj_list.add(
                        outbound_rtp_json.getJsonObject(outbound_rtp_stream));
                }
                break;
              }
            case "inbound-rtp":
              {
                ...
                for (String inbound_rtp_stream : inbound_rtp_stream_list) {
                  if (inbound_rtp_stream.toLowerCase().contains("inbound")
                      && inbound_rtp_stream.toLowerCase().contains(this.mediaType))
  ...
  private List<Integer> getPacketsLostOvertime() {
    ...
    for (JsonObject jsonObject : this.inbound_obj_list) {
      ...
      if (temp.equalsIgnoreCase("N/A") || temp.equalsIgnoreCase("NA")) res.add(0);
      else res.add(Integer.parseInt(temp));
    }
    if (res.size() > 0) res.remove(0);
    return res;
  }
  ...
  private List<Double> getJitterOvertime() {
    ...
    for (JsonObject jsonObject : this.inbound_obj_list) {
      ...
      if (temp.equalsIgnoreCase("N/A") || temp.equalsIgnoreCase("NA")) res.add(0.0);
      else res.add(Double.parseDouble(temp));
    }
    if (res.size() > 0) res.remove(0);
    return res;
  }
  ...
  private double getAvgBytesReceived() {
    ...
    for (int bytesReceived : tmp) sum += bytesReceived;
  ...
  private double getAvgBytesSent() {
    ...
    for (int avgBytesSent : tmp) sum += avgBytesSent;
  ...
  private double getAvgJitter() {
    ...
    for (double avgJitter : tmp) sum += avgJitter;
  ...
  private double getAvgPacketsLost() {
    ...
    for (int avgPacketsLost : tmp) sum += avgPacketsLost;
  ...
  private double getAvgPacketsReceived() {
    ...
    for (int avgPacketsReceived : tmp) sum += avgPacketsReceived;
  ...
  private double getAvgPacketsSent() {
    ...
    for (int avgPacketsSent : tmp) sum += avgPacketsSent;
  ...
  public String getJsonData() {
    ...
    if (getJitterOvertime().size() > 0) {
      ...
      for (int i = 0; i < this.getJitterOvertime().size(); i++) {
        ...
        if (i < this.getJitterOvertime().size() - 1) jsonData += ",";
      }
      jsonData += "],";
    }
