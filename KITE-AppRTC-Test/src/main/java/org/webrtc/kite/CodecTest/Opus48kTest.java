...
/**
 * Opus48KTest implementation of KiteTest.
 * <p>
 ...
 * stashed earlier.</li>
 * </ul>
 * <li>4) The test is considered as successful if the bitrate of opus is changed in the sdp.</li>
 * <li>5) A successful test returns a boolean 'true' while the unsuccessful test returns a boolean
 * 'false'.</li>
 * </ul>
 * </p>
 */
public class Opus48kTest extends KiteTest {

  private final static Logger logger = Logger.getLogger(Opus48kTest.class.getName());
  ...
  private final String value = "opus/48000";
  private final String option = "arc=" + value;
  ...
  private boolean validateResults(String result) {
    ...
    if (result.equalsIgnoreCase("unknown"))
      ...
      if (opusRTPMapNo.equalsIgnoreCase("unknown"))
        return false;
      for (String fmtp : fmtpList) {
        if (fmtp.contains(opusRTPMapNo))
          if (fmtp.contains("maxplaybackrate=48000"))
            return true;
      }
      return false;
    }
