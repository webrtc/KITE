            ...
            if (this.checkForFailed(resultList)) {
                ...
                if (resultList.contains("FAILED")) {
                    if (!resultList.contains("SUCCESSFUL"))
                        resultMap.put("result", "FAILED for both of the browsers (ARBR at " + BRList.get(0) + "kbps and " + BRList.get(1) + "kbps)");
                    else {
                        // Since for now apprtc can only support 2 browsers at a time
                        if (resultList.indexOf("FAILED") == 0)
                            resultMap.put("result", "FAILED for the caller (ARBR at " + BRList.get(0) + "kbps)");
                        else
                            resultMap.put("result", "FAILED for the callee (ARBR at " + BRList.get(1) + "kbps)");
                    }
                } else
