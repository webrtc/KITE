...
function displayScoreboardFirstLayer(){
    ...
    var firstRow = '<tr><th><a class="medium-boy" href="#">Home</a></th>';
    var content = '';
    var listOfTestSuite = [];
    ...
    listOfTestSuite.forEach(function(testSuite){
        content += '<tr><td class="test-name"><a href="?testsuite='+testSuite+'" class="small-boy" data-toggle="tooltip" data-placement="top" title="'
                       +scoreJson[''+listOfBrowsers[0]].results[testSuite].description+'">' + testSuite + '</a></td>';
        listOfBrowsers.forEach(function(browser){
            var browserJson = scoreJson[browser];
            var browserResultJson = browserJson.results;
            ...
            if (total === 0)
                style += '160,160,160); color:black;"';
            else {
                if (pass === 0)
                    style += '244,90,90); color:white;"';
                else {
            ...
            content += '<td align="center" id="'+browser+'"  '+style+' ><a href="?testsuite='+testSuite+'" class="small-boy">' + pass+'/'+ total + '</a></td>'
        });
    });
...
function displayScoreboardMiddleLayers(){
    var scoreboardHead = $('#scoreboardHead');
    var scoreboardBody = $('#scoreboardBody');
    var firstRow = '<tr><th><a class="small-boy" href="ready-yet">Home</a></br>/<a class="medium-boy" href="?testsuite='
                +testSuite+'">'+testSuite+'</a></th>';
    var content = '';
    ...
    listOfBrowsers.forEach(function(browser){
        ...
        var browserResultJson = browserJson.results;
        var browserTestResult = browserResultJson[testSuite];
        var subTests = browserTestResult.tests;
        var browserTestList = Object.keys(subTests);
...
function displayScoreboardLastLayer(){
    var scoreboardHead = $('#scoreboardHead');
    var scoreboardBody = $('#scoreboardBody');
    var firstRow = '<tr><th><a class="small-boy" href="ready-yet">Home</a></br>/<a class="small-boy" href="?testsuite='
    ...
    listOfBrowsers.forEach(function(browser){
        ...
        var browserResultJson = browserJson.results;
        var browserTestResult = browserResultJson[testSuite];
        var browserTestList = browserTestResult.tests;
        var browserTestResult = browserTestList[''+test].result.result.tests;
        var subTestList = Object.keys(browserTestResult);
        testList = subTestList;
        subTestList.forEach(function(subTest){
            var subTestResult = browserTestResult[subTest];
